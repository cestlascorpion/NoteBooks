...
只在move ctor和swap上加noexcept。不建议业务代码使用noexcept。noexcept与优化相关，不影响代码正确性；判断复杂，容易出错；影响接口灵活性，比如虚函数设置为noexcept，派生类覆写不一定能保证；C++17后，noexcept还影响了函数的签名，进一步影响了代码的复杂性和兼容性。
